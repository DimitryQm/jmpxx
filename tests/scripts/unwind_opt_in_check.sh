...
# jmpxx/unwind.hpp; no portable public header may pull it in, so a consumer who never
# opts in never compiles it. This reads every public header outside the arm's own
...
echo "unwind opt-in OK: no default-path public header includes the unwind arm"
